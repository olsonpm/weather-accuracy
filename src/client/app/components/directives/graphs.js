...
module.exports = function(app, log) {
  app.directive('graphs', function() {
    var linkFn = function linkFn(scope, element, attrs, interactiveGraphCtrl) {
      ...
      var svg = {}
        , leftMargin = calculateResponsiveLength(25)
        , commonMargin = calculateResponsiveLength(10)
        , translateX = roundn((leftMargin + commonMargin) / 2, 0)
        , translateY = roundn(commonMargin / 3, 0)
        , aggregateBarWidth = calculateResponsiveLength(10)

        , width = elementWidth - (leftMargin + commonMargin)
        , height = calculateResponsiveLength(GRAPH_HEIGHT) - (commonMargin * 2)

        , parseDate = d3.time.format("%Y%m%d").parse
        , dateOutputFormat = d3.time.format('%_d')

        , x = d3.time.scale()
          .range([0, width])
        , y = d3.scale.linear()
          .range([height, 0])
        , xAggregate = d3.scale.linear()
          .range([0, width])
        , yAggregate = y
        ...
        , xPos = function(d) { return x(d.ymd); }
        , actualY = function(d) { return y(d.actual); }
        , forecastedY = function(d) { return y(d.forecasted); }
    ...
    return {
      ...
      , link: linkFn
      , scope: {}
      , templateUrl: 'app/components/directives/graphs.html'
    };
  });
};
