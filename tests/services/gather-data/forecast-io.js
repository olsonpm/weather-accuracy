...
var GatherFioData = require('../../../services/gather-data/forecast-io')
    ...
    , testForecast = require('./example/fio-forecast')
    , testActual = require('./example/fio-actual')
    , chai = require('chai')
...
suite("gather_fio_data", function gather_fio_data() {
    ...
    var tomorrowForecast = lazy(testForecast.daily.data)
        .mustFind(function(aForecastDay) {
            var tmpMoment = moment(aForecastDay.time, 'X');
    ...
    setup(function() {
        return new GatherFioData(pgWrapInst).bInit()
            .then(function(res) {
                gatherInst = res;
    ...
    test("sanitize_5_day_forecast", function sanitize_5_day_forecast() {
        return bPromise.all(GatherFioData._sanitize5DayForecastData(bPromise.resolve(JSON.stringify(testForecast)), richmond, gatherInst))
            .then(function(forecastArray) {
                ...
                for (var i = 0; i < 15; i++) {
                    ...
                    assert.strictEqual(tmpData.Source().Name(), Source.NAMES.FIO);
    ...
    test("get_value_and_measurement_name", function get_value_and_measurement_name() {
        ...
        var res = GatherFioData._getValueAndMeasurementName(tomorrowForecast, 0, curType, gatherInst);
        ...
        res = GatherFioData._getValueAndMeasurementName(tomorrowForecast, 1, curType, gatherInst);
        assert.strictEqual(res.curMeasurementName.Value(), MeasurementName.VALUES.LOW_TEMP);
        assert.isTrue(Utils.isNumeric(res.curValue));
        res = GatherFioData._getValueAndMeasurementName(tomorrowForecast, 2, curType, gatherInst);
        ...
        res = GatherFioData._getValueAndMeasurementName(actualData, 0, curType, gatherInst);
        assert.strictEqual(res.curMeasurementName.Value(), MeasurementName.VALUES.HIGH_TEMP);
        assert.isTrue(Utils.isNumeric(res.curValue));
        res = GatherFioData._getValueAndMeasurementName(actualData, 1, curType, gatherInst);
        assert.strictEqual(res.curMeasurementName.Value(), MeasurementName.VALUES.LOW_TEMP);
        assert.isTrue(Utils.isNumeric(res.curValue));
        res = GatherFioData._getValueAndMeasurementName(actualData, 2, curType, gatherInst);
        assert.strictEqual(res.curMeasurementName.Value(), MeasurementName.VALUES.MEAN_WIND_SPEED);
        assert.isTrue(Utils.isNumeric(res.curValue));
    });
    ...
    test("get_data_point_promise", function get_data_point_promise() {
        ...
        return bPromise.join(
            GatherFioData._getDataPointPromise(bTestForecastData, tomorrowForecast, 0, gatherInst)
            , GatherFioData._getDataPointPromise(bTestActualData, actualData, 0, gatherInst)
            , assertDataPoints
        );
    ...
    test("get_type", function get_forecast_type() {
        var testType = GatherFioData._getType(tomorrowMoment, gatherInst);
        assert.strictEqual(testType.Name(), Type.NAMES.FORECAST_DAY_1);

        testType = GatherFioData._getType(actualMoment, gatherInst);
        assert.strictEqual(testType.Name(), Type.NAMES.ACTUAL);
    });

    test("get_ymd", function get_ymd() {
        var testYmd = GatherFioData._getYmd(tomorrowMoment, gatherInst);
        assert.strictEqual(testYmd.Value(), tomorrowMoment.format('YYYYMMDD'));

        testYmd = GatherFioData._getYmd(actualMoment, gatherInst);
    ...
    test("get_forecast_data_point_data_promise", function get_forecast_data_point_data_promise() {
        return GatherFioData._getForecastDataPointDataPromise(tomorrowForecast, richmond, gatherInst)
            .then(function(aData) {
                ...
                assert.strictEqual(aData.Source().Name(), Source.NAMES.FIO);
    ...
    test("filter_days_forecasted", function filter_days_forecasted() {
        var filteredForecastDays = lazy(testForecast.daily.data)
            .filter(function(aForecastDay) {
                return GatherFioData._filterDaysForecasted(aForecastDay, gatherInst);
            })
            .sort(function(left, right) {
                var leftMoment = moment(left.time, 'X');
                var rightMoment = moment(right.time, 'X');
        ...
        for (var i = 0; i < 5; i++) {
            ...
            tmpMoment = moment(tmpFDay.time, 'X');
    ...
    test("sanitize_actual_data", function sanitize_actual_data() {
        return bPromise.all(GatherFioData._sanitizeActualData(bPromise.resolve(JSON.stringify(testActual)), richmond, gatherInst))
    ...
    test("get_actual_data_point_data_promise", function get_actual_data_point_data_promise() {
        return GatherFioData._getActualDataPointDataPromise(actualData, richmond, gatherInst)
            .then(function(aData) {
                assert.strictEqual(aData.Location().Name(), Location.NAMES.RICHMOND);
